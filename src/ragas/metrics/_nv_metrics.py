...
class ResponseGroundedness(MetricWithLLM, SingleTurnMetric):
    ...
    # CIT: added rules 0a (disclaimer) and 0b (citation) before original A-D.
    ...
    template_groundedness1 = (
        ...
        "The context may begin with a source header of the form `[Source: <filename>, page <N>]`.\n"
        "Your task is to determine if the assertion is supported by the context.\n\n"
        "Follow the instructions below, in order:\n\n"
        "0a. (Disclaimer case.) If the assertion is primarily a disclaimer — "
        "explicitly declines to answer, apologizes, or states the information is "
        "absent / unknown / out of scope (e.g. \"申し訳ございません、データにありません\", "
        "\"I don't have information on this\"):\n"
        "   - Say 2 if the context does not contain the data the assertion claims "
        "is absent (correct refusal, fully grounded in the true state of retrieved "
        "evidence).\n"
        "   - Say 0 if the context clearly does contain the data the assertion "
        "claims is absent.\n"
        "   - Skip rules A-D in this case.\n\n"
        "0b. (Citation case.) If the assertion is primarily a source reference — "
        "cites a specific filename, page number, or section (e.g. \"see foo.pdf "
        "page 6\", \"詳細は bar.pdf 10ページ\") AND the context's `[Source: ...]` "
        "header matches that filename / page:\n"
        "   - Say 2.\n"
        "   - Skip rules A-D in this case.\n"
        "   - If no context has a `[Source: ...]` header at all, fall through to "
        "rules A-D.\n\n"
    ...
    template_groundedness2 = (
        ...
        "I will evaluate the level of support an assertion receives from the provided context.\n\n"
        "The context may begin with a source header of the form `[Source: <filename>, page <N>]`; "
        "treat this as structured metadata.\n\n"
        "Special cases (check these first):\n\n"
        "* Disclaimer assertion. If the assertion is primarily a disclaimer "
        "(explicitly declines to answer, apologizes, or states information is "
        "absent / unknown — e.g. \"申し訳ございません、データにありません\", "
        "\"I don't have information on X\"):\n"
        "    - Assign 2 if the context does not contain the data the assertion "
        "claims is absent.\n"
        "    - Assign 0 if the context clearly contains that data.\n"
        "    - Skip the general guidelines below.\n\n"
        "* Citation assertion. If the assertion is primarily a source reference "
        "(filename / page / section) AND the context's `[Source: ...]` header "
        "matches the citation:\n"
        "    - Assign 2.\n"
        "    - Skip the general guidelines below.\n"
        "    - If no context carries a source header, fall through.\n\n"
        "General guidelines (apply when neither special case fires):\n"
